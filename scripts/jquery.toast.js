(function($) {
	var th = null, cf = null, toast = function(m, o) {
		...
		if (!th) {
			// get/fix config
			cf = o;
			th = $('<ul></ul>').addClass('toast').appendTo($(cf.base)).hide();
			...
			th.width(cf.width);
			(cf.align === 'left' || cf.align === 'right') && th.css('margin', '5px').css(cf.align, '0') || th.css({
				left : '50%',
				margin : '5px 0 0 -' + (cf.width / 2) + 'px'
			});
		}
		// create toast
		...
		!o.sticky && o.duration > 0 && (to = setTimeout(function() {
			cb.click();
		}, o.duration));
		// show toast
		ti.fadeIn('normal');
